...
int main() {
    ...
    server.sin_addr.s_addr = inet_addr("192.168.1.130"); // <- ESP32's IP
    ...
    {
        pixel[i].color = (struct COLOR){i*2, 250, 10};
        pixel[i].modo = PULSO;
        pixel[i].tiempo = 0;
        pixel[i].offset = i*50;
        pixel[i].extra = 30;
        pixel[i].params.respiracion.t_apagar = 1500;
        pixel[i].params.respiracion.t_encender = 1500;
